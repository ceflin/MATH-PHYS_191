...
namespace Lab1_TheVectorClass
{
    class Vector3D
    {
        ...
        #region 3D calculations

        public Vector3D normalized(Vector3D vector)
        {
            float mag = CalcMag(vector.x, vector.y, vector.z);
        ...
        //3D perpProj
        public Vector3D prepProj(Vector3D U, Vector3D V)
        {
            Vector3D parallel = paraProj(U, V);
            return U - parallel;
        }

        //3D Cross product
        public float crossProd(Vector3D U, Vector3D V)
        {
            return ((U.y * V.z) - (V.y * U.z)) - ((U.x * V.z) - (V.x * U.z)) + ((U.x * V.y) - (V.x * U.y));
        }

        //Closest point on a line using parallel projection
        //where P = a point on a line and V = the parallel vector to said line
        //and Q = 
        //and the vector this function is called on is the point away from the line.
        public Vector3D closestPointLine(Vector3D P, Vector3D V)
        {
